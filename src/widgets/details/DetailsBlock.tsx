...
import { getBlock, useBlockQuery } from "../../entities/block";
...
//   const data = mapBlockToTableElement(block);
//   return {
//     data,
//   };
// }

// const useDetailsBlockQuery = (id: string = "latest"):QueryObserverResult<TableElementBlock, AxiosError> => {
//   return useQuery({
...
const DetailsBlock = () => {
  const { id } = useParams();
  // const {data: block, isFetching, isError} = useDetailsBlockQuery(id);
  const [block, setBlock] = useState<TableElementBlock | undefined  >();

  useEffect(() => {
    const FetchBlock = async () => {
      try {
        const blockQueryResult = await useBlockQuery(id!);
        const ready = mapBlockToTableElement(blockQueryResult.data!);
        setBlock(ready);
      } catch (error) {
  ...
  //       <h1 className="text-secondary">Error occures</h1>
  //     </div>
  // }
    ...
    <section className="text-gray-300 bg-black bg-opacity-50">
      <div className="px-10 py-5 space-y-5 mx-auto">
        <div className="md:mb-0 mb-6 flex font-semibold flex-col md:flex-row">
          ...
          <p className="text-white md:basis-9/12">{block?.blockNumber?.toString()}</p>
        </div>
        <div className="md:mb-0 mb-6 flex font-semibold flex-col md:flex-row">
          <span className="md:basis-3/12">
            Timestamp:
        ...
        <div className="md:mb-0 mb-6 flex font-semibold flex-col md:flex-row">
          ...
          <p className="text-white md:basis-9/12">{block?.earnedFees?.toString()}</p>
        </div>
        <div className="md:mb-0 mb-6 flex font-semibold flex-col md:flex-row">
          <span className="md:basis-3/12">
            Summary Value:
