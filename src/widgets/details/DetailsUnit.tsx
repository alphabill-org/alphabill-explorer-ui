import { useTxsByUnitIDQuery } from "../../entities/tx";
import { DetailProps } from "./types";


const DetailsUnit = ({ id }: DetailProps) => {
  const { data, isFetching } = useTxsByUnitIDQuery(id);
  ...
  return (
    <section className="flex flex-col text-gray-300 bg-black bg-opacity-50">
      {data?.map((tx) => (
        <div>{tx.TxRecordHash}</div>
      ))}
    </section>
  );
};
